...
class SimpleNoAccessErrorHandler implements RequestErrorHandler
{
	...
	public function handle(\Octarine\Routing\Request\Request $request, \Octarine\Access\Principal $requester, $params)
	{
		switch ($request->matchContentType(array('text/html', 'application/json', 'text/plain')))
		{
			case 'text/html':
				...
				$site->setWhereabouts('401');
				$site->appendElement('h1')->appendText('Authentication Required');
				$site->appendElement('p')->appendText('You have to authenticate before you can access this resource.');
				return new \Octarine\Routing\Response\HtmlResponse(401, $site->getDocument());
			case 'application/json':
				return new \Octarine\Routing\Response\JsonResponse(401, array('error_msg' => 'Authentication required.'));
			default:
				return new \Octarine\Routing\Response\TextResponse(401, 'Authentication required');
		}
	}
}
