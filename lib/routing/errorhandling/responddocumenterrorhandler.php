...
class RespondDocumentErrorHandler implements RequestErrorHandler
{
	public function __construct(\Octarine\Document\HtmlDocument $document, $status_code = 200)
	{
		$this->document = $document;
		$this->status_code = $status_code;
	}

	private $document, $status_code;

	public function handle(\Octarine\Routing\Request\Request $request, \Octarine\Access\Principal $requester, $params)
	{
		// Do not use cache control on error pages
		return new \Octarine\Routing\Response\HtmlResponse($this->document, $this->status_code, false);
	}
}
