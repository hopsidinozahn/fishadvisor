...
class SimpleExceptionRenderer implements ExceptionRenderer
{
	...
	{
		return $this->buildHtml('Octarine CMS Error', $exception->getMessage());
	}

	private function buildHtml($title, $exception_message)
	{
...
</html>', htmlspecialchars($title), $this->buildCss(), htmlspecialchars(strval($exception_message)));
	}
