...
class DbPasswordPrincipalManager implements PrincipalManager
{
	public function __construct(\PDO $connection, $table_users)
	{
		...
		$this->stmt_auth = $connection->prepare(sprintf('SELECT * FROM `%s` WHERE `name`=? AND `password`=MD5(CONCAT(?, `salt`))', $table_users));
	...
	{
		list($authentication_name, $authentication_password) = func_get_args();
		if (!$this->stmt_auth->execute(array($authentication_name, $authentication_password)))
		{
			$err = $this->stmt_auth->errorInfo();
		...
		if (!$o)
			...
			return new DbPrincipal($o->id, $o->name, $o->access);
	}
