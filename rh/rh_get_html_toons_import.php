...
class rh_get_html_toons_import extends \Octarine\Routing\Request\RequestHandlerBase
{
	...
	public function handle(\Octarine\Routing\Request\ValidatedRequest $request)
	{
		...
		$site->setTitle('Import toons'); // already i18n
		$site->document->addScript('/js/export.min.js');
		$site->document->addInlineScript('$(function(){init_export("","");});');
		...
		$site->appendElement('h2')->appendText($this->localizer->get('Import toons'));
		$site->appendElement('section')->setAttr('id', 'toons_import_control')->setAttr('data-import', $request->getParameters()->get('data'));
