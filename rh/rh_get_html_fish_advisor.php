...
class rh_get_html_fish_advisor extends \Octarine\Routing\Request\RequestHandlerBase
{
	...
	public function handle(\Octarine\Routing\Request\ValidatedRequest $request)
	{
		...
		$site->setTitle('Species advisor'); // already i18n
		$site->noMargin();
		$site->document->addInlineScript(getInitJavascript($this->engine, $rod->id));

		// Render site navigation

		renderNavigation($this->engine, $this->localizer, $site, $rod->id, 'fish_advisor', 'advisor');

		// Render page

		$header = $site->appendElement('header')->setAttr('class', 'advisor');
		$header->appendElement('h1')->appendText($this->localizer->get('Species advisor'));
		$header->appendElement('p')->appendText($this->localizer->get('What species should I catch before buying the next rod?'));
		$header->appendElement('p')->setAttr('id', 'toon_selection')->appendText($this->localizer->get('Giving advice to:'));
		$site->appendElement('section')->setAttr('id', 'species_advisor_control');
