...
class rh_get_html_pond_advisor extends \Octarine\Routing\Request\RequestHandlerBase
{
	...
	public function handle(\Octarine\Routing\Request\ValidatedRequest $request)
	{
		...
		$site->setTitle('Pond advisor'); // already i18n
		$site->noMargin();
		$site->document->addInlineScript(getInitJavascript($this->engine, $rod->id));

		// Render site navigation

		renderNavigation($this->engine, $this->localizer, $site, $rod->id, 'pond_advisor', 'advisor');

		// Render page

		$header = $site->appendElement('header')->setAttr('class', 'advisor');
		$header->appendElement('h1')->appendText($this->localizer->get('Pond advisor'));
		$header->appendElement('p')->appendText($this->localizer->get('Where should I fish if catching a new species shall be as likely as possible?'));
		$header->appendElement('p')->setAttr('id', 'toon_selection')->appendText($this->localizer->get('Giving advice to:'));
		$site->appendElement('section')->setAttr('id', 'pond_advisor_control');
