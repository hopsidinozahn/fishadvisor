...
class rh_get_json_fish_advisor extends \Octarine\Routing\Request\RequestHandlerBase
{
	...
	public function handle(\Octarine\Routing\Request\ValidatedRequest $request)
	{
		...
		$stats = $stats_computer->getFishesByProbability($rod->id, $interested_fish_list);
		...
		foreach ($stats as $entry)
		{
			...
			foreach ($entry->advice as $advice_entry)
			{
				if (++$i > 3) break;
				$ponds[] = array(
					'id' => $advice_entry->pond->id,
					'name' => $advice_entry->pond->name,
